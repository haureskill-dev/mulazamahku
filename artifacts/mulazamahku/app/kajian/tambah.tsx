...
export default function TambahJadwalScreen() {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    const { success, error } = await KajianTambahanService.create({
      ...form,
      ustadz: "Ustadzah Rubeya Litiloly",
      created_by_email: user?.email || "unknown@domain.com",
      created_by_role: user?.role || "pengajar"
    });
    setLoading(false);

    if (success) {
      Alert.alert("Berhasil", "Jadwal kajian berhasil ditambahkan.", [
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background, paddingTop: insets.top }]}>
      <View style={[styles.header, { borderBottomColor: colors.border }]}>
        ...
        <Text style={[styles.title, { color: colors.foreground }]}>Tambah Jadwal Kajian</Text>
        <View style={{ width: 24 }} />
      </View>

      <ScrollView contentContainerStyle={styles.content}>
        <Text style={[styles.infoText, { color: colors.mutedForeground }]}>
          Jadwal ini akan muncul di daftar kajian lengkap bagi semua pengguna. Cocok untuk kajian privat atau khusus lingkungan tertentu.
        </Text>
